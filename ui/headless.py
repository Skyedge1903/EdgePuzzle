...
class BoardUi:
    ...
    def init(self):
        os.environ["SDL_VIDEODRIVER"] = "windib"  # Windows fallback
        pygame.init()
        pygame.font.init()
        ...
        color_images = {}
        for i in range(1, 23):
            color_images[i] = pygame.image.load(
                f"data/patterns/pattern{i}.png"
            ).convert_alpha()
        ...
        for id, piece in self.board.puzzle_def.all.items():
            ...
            if color_id != 0:
                high_res.blit(pygame.transform.rotate(color_images[color_id], 0), (0, 0))
            color_id = piece.colors[3]
            if color_id != 0:
                high_res.blit(pygame.transform.rotate(color_images[color_id], 270), (color_dim, 0))
            color_id = piece.colors[1]
            if color_id != 0:
                high_res.blit(pygame.transform.rotate(color_images[color_id], 90), (0, color_dim))
            color_id = piece.colors[0]
            if color_id != 0:
                high_res.blit(pygame.transform.rotate(color_images[color_id], 180), (color_dim, color_dim))
    ...
    def draw(self, piece, x, y):
        border_points = [
            (x, y),
            (x, y + self.piece_width),
            (x + self.piece_width, y + self.piece_width),
            (x + self.piece_width, y)
        ]
